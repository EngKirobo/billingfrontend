...
@Component({
  ...
  templateUrl: './assignrooms.html',
  styleUrl: './assignrooms.css',
})
export class AssignroomsComponent implements OnInit {
  details: HostelDetail[] = [];
  detailz: any[] = [];
  filteredDetails: any[] = []; // 👈 this is what table will use
  form!: FormGroup;
  intakes: Intake[] = [];
  hostels: Hostel[] = [];
  isEdit = false;
...
loadSavedForm() {
  ...
  this.service.getAll().subscribe(data => {
    this.detailz = data;
    this.filteredDetails = data; // 👈 default = all
  });
}

filterByHostel(hostelId: number | null) {
  if (!hostelId) {
    this.filteredDetails = this.detailz;
  } else {
    this.filteredDetails = this.detailz.filter(
...
applyFilters() {
  const { hostelId, levelId, intakeId } = this.form.value;

  this.filteredDetails = this.detailz.filter(d => {

    const matchHostel = hostelId ? d.hostelId == hostelId : true;
    const matchLevel  = levelId  ? d.levelId  == levelId  : true;
    const matchIntake = intakeId ? d.intakeId == intakeId : true;

    return matchHostel && matchLevel && matchIntake;
  });
}
